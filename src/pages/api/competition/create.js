...
export default async function handler(req, res) {
    ...
    if (method === 'POST') {
        ...
        if(new Date(data.endInscriptionDate).getTime() < new Date(data.startInscriptionDate).getTime() ||
        ...
        /*let insert = await db.query('INSERT INTO COMPETITION(NAME,DESCRIPTION,START_INSCRIPTION,END_INSCRIPTION,START_DATE,END_DATE,TEAM_MEMBERS_MIN,TEAM_MEMBERS_MAX) ' +
                                    'VALUES($1,$2,TO_TIMESTAMP($3),TO_TIMESTAMP($4),TO_TIMESTAMP($5),TO_TIMESTAMP($6),$7,$8, '6')',
                                    [data.name, data.description, date[0], date[1], date[2], date[3], data.minMembers, data.maxMembers]);  */        
